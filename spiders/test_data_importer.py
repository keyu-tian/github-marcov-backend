...
def test_data_import():
    ...
    for i in range(1+question_num, 11+question_num):
        q = Question.objects.create(title=f'question{i}', user=users[random.randint(0, len(expert_name)-1)], published_time=datetime.now().strftime('%Y-%m-%d %H:%M:%S'), solved=random.randint(1, 10) > 8)
        Content.objects.create(user=q.user, published_time=datetime.now().strftime('%Y-%m-%d %H:%M:%S'), content=rand_content(), floor=1, question=q, is_top=random.randint(1, 100) > 90)
        questions.append(q)
    ...
    for i in range(1, 100):
        c = Content.objects.create(user=users[random.randint(0, len(expert_name)-1)], content=rand_content(), floor=0, published_time=datetime.now().strftime('%Y-%m-%d %H:%M:%S'), question=questions[random.randint(0, 9)], is_top=random.randint(1, 100) > 90)
        c.question.replied_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
