    ...
    for a in range(index, len(mingming)):
        for i in range(st, mingming[a][1]):
            ...
            if result and not Train.objects.filter(name=name).exists():
                ...
                train, flag = Train.objects.get_or_create(
                    name=name, defaults={
                        'dept_date': dept_date,
                        'dept_time': dept_time,
                        'arri_date': arri_date,
...
if __name__ == '__main__':
    ...
    parser.add_argument('index', default=0, type=str, required=True)
    parser.add_argument('st', default=1, type=str, required=True)
